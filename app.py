...
with tab1:
    ...
    with c1:
        ...
        st.info(f"💡 **AI 信心度:** {abs(final_score - 50) * 2:.0f}%")
    ...
    with c2:
        ...
        if plan:
            if not plan['is_allowed']:
                ...
                k1.metric("建议挂单", f"${plan['entry']:.2f}", plan['dir'])
                k2.metric("止盈目标", f"${plan['tp']:.2f}")
                k3.metric("止损防守", f"${plan['sl']:.2f}", delta_color="inverse")
